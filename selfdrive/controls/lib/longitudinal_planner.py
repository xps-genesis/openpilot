...
_A_CRUISE_MIN_V = [-.3, -.3, -.3, -.3, -.3]
_A_CRUISE_MIN_BP = [  0.,  5.,  10., 20.,  40.]

# need fast accel at very low speed for stop and go
# make sure these accelerations are smaller than mpc limits
_A_CRUISE_MAX_V = [1.0, 2.0, 1.5, .8, .4, .15]
_A_CRUISE_MAX_V_FOLLOWING = [1.0, 2.0, 1.5, .8, .4, .15]
_A_CRUISE_MAX_BP = [0., .8,  6.4, 18., 22.5, 40.]
...
class Planner():
  ...
  def update(self, sm, CP, lateral_planner):
    ...
    if enabled and not self.first_loop and not sm['carState'].accgasOverride:
